// input field
  ...
  if (name1.value.trim() === '' || name2.value.trim() === '') {
    ...
    localStorage.setItem('player1', name1.value.trim());
    localStorage.setItem('player2', name2.value.trim());
  }
}
...
async function fetchCategories(){
  ...
  document.getElementById('category_fetch_button').style.display = 'none';
} 

function displaycategories(categories) {
  const categoriesSection = document.getElementById('category_selection');
  categoriesSection.innerHTML = `<option value="" disabled selected>Select a Category</option>`;
  let options = '';
  Object.keys(categories).forEach(category => {
    options += `<option value="${category}">${categories[category][0]}</option>`;
  });
  categoriesSection.innerHTML += options;
}
...
function showQuestion() {
  ...
  options.forEach((option) => {
    ...
    button.onclick = () => {
      answerEvaluation(question, button, option);
      questionIndex++;
    };
...
function answerEvaluation(question, button, selectedOption) {
...
function continueGame() {
  ...
  if (categoryCount === 0) {
    alert('There are no more categories to continue the game.');
    document.getElementById('categories_container').style.display = 'none';
    document.getElementById('questionsContainer').style.display = 'none';
    endGame();
    return;
...
function endGame() {
  ...
  document.getElementById(
    'player1-final-score'
  ).textContent = `${player1}: ${player1Score} points`;
  document.getElementById(
    'player2-final-score'
  ).textContent = `${player2}: ${player2Score} points`;
